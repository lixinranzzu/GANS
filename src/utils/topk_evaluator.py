...
class TopKEvaluator(object):
    ...
    def evaluate(self, batch_matrix_list, eval_data, is_test=False, idx=0, use_split=False, split_user=None):
        ...
        ndcg_group_dict  = {}
        if use_split:
            result_list, group_result = self._calculate_metrics(pos_len_list, bool_rec_matrix, use_split, split_user)
            ...
            for i in range(len(group_result[0])):
                key = '{}'.format(i+1)
                recall_group_dict[key] = round(group_result[0][i], 4)
            for i in range(len(group_result[1])):
                key = '{}'.format(i+1)
                ndcg_group_dict[key] = round(group_result[1][i], 4)
            return metric_dict, recall_group_dict, ndcg_group_dict
        else:
            result_list = self._calculate_metrics(pos_len_list, bool_rec_matrix, use_split, split_user)
            for metric, value in zip(self.metrics, result_list):
                for k in self.topk:
                    key = '{}@{}'.format(metric, k)
                    metric_dict[key] = round(value[k - 1], 4)
    ...
    def _calculate_metrics(self, pos_len_list, topk_index, use_split=False, split_user=None):
        ...
        for metric in self.metrics:
            ...
            if (metric.lower() =="ndcg" or metric.lower() =="recall") and use_split:
                result, group = metric_fuc(topk_index, pos_len_list, use_split, split_user)
                group_result.append(group)
            else:
                result = metric_fuc(topk_index, pos_len_list, use_split)

            result_list.append(result)
        if use_split:
            return np.stack(result_list, axis=0), np.stack(group_result, axis=0)
        else:
