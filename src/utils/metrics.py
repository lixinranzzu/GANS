...
def recall_(pos_index, pos_len, use_split=False, split_user=None):
    # Recall: average single users recall ratio.
    rec_ret = np.cumsum(pos_index, axis=1) / pos_len.reshape(-1, 1)
    if use_split:
        result_split = []
        for i in range(4):
            users = np.array(split_user[i], dtype=int).flatten()
            split_recall = rec_ret[users].mean(axis=0)
            result_split.append(split_recall[19])
        return rec_ret.mean(axis=0), result_split
    return rec_ret.mean(axis=0)


def recall2_(pos_index, pos_len, use_split=False,  split_user=None):
    r"""
...
def ndcg_(pos_index, pos_len, use_split=False,  split_user=None):
    ...
    if use_split:
        ...
        for i in range(4):
            users = np.array(split_user[i], dtype=int).flatten()
            split_ndcg = result[users].mean(axis=0)
            result_split.append(split_ndcg[19])
        return result.mean(axis=0), result_split
    return result.mean(axis=0)


def map_(pos_index, pos_len, use_split=False, result_split=None):
...
def precision_(pos_index, pos_len, use_split=False, result_split=None):
